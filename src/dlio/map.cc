...
dlio::MapNode::MapNode(): Node("dlio_map_node") {
  ...
  this->deskewed_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>("/dlio/odom_node/pointcloud/deskewed", 10,
      std::bind(&dlio::MapNode::callbackDeskewed, this, std::placeholders::_1), keyframe_sub_opt);
...
void dlio::MapNode::callbackDeskewed(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& deskewed) {

  // convert scan to pcl format
  pcl::PointCloud<PointType>::Ptr deskewed_pcl = std::make_shared<pcl::PointCloud<PointType>>();
  pcl::fromROSMsg(*deskewed, *deskewed_pcl);
  // save filtered keyframe to map for rviz
  *this->dlio_map += *deskewed_pcl;
